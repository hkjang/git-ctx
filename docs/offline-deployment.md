...
GitHub Release의 `git-ctx-v0.1.0-linux-amd64.tar.gz`는 네트워크가 없는 Linux
...
```bash
sha256sum -c git-ctx-v0.1.0-linux-amd64.tar.gz.sha256
gzip -dc git-ctx-v0.1.0-linux-amd64.tar.gz | docker load
docker image inspect git-ctx:v0.1.0 --format '{{.Id}} {{.Architecture}} {{.Os}}'
```
...
```bash
gzip -dc git-ctx-v0.1.0-linux-amd64.tar.gz \
  | sudo ctr --namespace k8s.io images import -
sudo ctr --namespace k8s.io images list | grep git-ctx
```

`deploy/kubernetes/base/deployment.yaml`의 이미지 이름을 `git-ctx:v0.1.0` 또는
