...
구분할 수 있습니다. 이미지 빌드 시 `--build-arg VERSION=v0.54.0` 을 주면 소스의
버전과 다를 때 빌드가 실패하므로, 태그와 코드가 어긋난 이미지가 만들어지지 않습니다.

릴리스 아카이브에는 정식 태그 `git-ctx:v0.54.0`와 호환 태그
`git-ctx:0.54.0`가 함께 들어 있습니다. 신규 배포 구성에는 정식 태그를 사용합니다.
...
```bash
scripts/verify-offline-image.sh 0.54.0 \
  git-ctx-v0.54.0.tar.gz <릴리스-커밋-SHA>
```
