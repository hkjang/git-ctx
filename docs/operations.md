...
필수 외부 값은 `GIT_CTX_DB_DSN`과 `GIT_CTX_RECOVERY_KEY` 두 개다. DB driver는 DSN에서
결정한다. 설정 암호화 키와 API-key pepper는 데이터베이스의 `platform_keys`에 저장되며
복구 키에서 파생한 키로 감싸여 있다. 복구 키는 복구 토큰 서명과 백업 봉인에도 쓰인다.
최소 32자의 고엔트로피 값으로 최초 한 번만 생성한다.
...
v0.66.0 이전에는 설정 암호화 키와 API-key pepper를 Bootstrap DSN 문자열에서 파생했고,
그래서 그 문자열을 바꾸면 설치를 열 수 없었다. 지금은 키가 데이터베이스에 저장되므로
DSN은 자유롭게 바꿀 수 있다 — 데이터 디렉터리 이동, 상대 경로를 절대 경로로, 질의
매개변수 순서 변경, PostgreSQL 호스트 이름 변경 모두 안전하다.

v0.66.0 이전에 만들어진 설치는 **한 번은 원래 DSN으로 기동**해야 한다. 그때 키가
데이터베이스에 기록되며, 그 뒤로는 제약이 없다.

### 복구 키

`GIT_CTX_RECOVERY_KEY`는 DSN이나 DB 비밀번호를 재사용하지 않는 별도 장기 Secret이며,
이 설치에서 잃어버리면 안 되는 **유일한** 값이다. 세 가지가 여기에 달려 있다.

- 설정과 API-key pepper를 감싸는 키
- 복구 토큰 서명
- 백업 아카이브 봉인 — 같은 복구 키를 가진 다른 설치가 아카이브를 열 수 있다는 뜻이고,
  이것이 다른 기기에서의 복원을 가능하게 한다

DB 백업·백업 볼륨과 **분리해** 복구 가능한 Secret Store에 보관한다. 백업과 같은 곳에
두면 아카이브 암호화가 의미를 잃는다. 모든 Pod와 `recovery-token` 명령에 같은 값을
제공한다.

### 복구 키 회전

회전은 한 번의 기동으로 끝난다. 새 키를 `GIT_CTX_RECOVERY_KEY`에, 직전 키를
`GIT_CTX_PREVIOUS_RECOVERY_KEY`에 넣고 기동하면 저장된 키 자료가 새 키로 다시 감싸진다.

```bash
GIT_CTX_RECOVERY_KEY='<새 장기 키>' GIT_CTX_PREVIOUS_RECOVERY_KEY='<직전 장기 키>' /app/git-ctx
```

- 직전 키를 주지 않으면 기동이 거부된다. 오류 문구가 이 변수를 안내한다.
- 회전 전에 만들어진 백업 아카이브는 **직전 키가 주어진 동안에만** 열린다. 회전 후에도
  옛 아카이브를 복원할 계획이면 직전 키를 보관하거나, 회전 직후 새 백업을 하나 만든다.
- 아직 만료되지 않은 복구 토큰은 무효가 되므로 전체 replica를 동시에 갱신하고 새 토큰을
  생성한다.
- 회전이 끝나면 `GIT_CTX_PREVIOUS_RECOVERY_KEY`를 제거한다.

### 재해 복구

백업 아카이브와 복구 키만 있으면 빈 데이터베이스의 새 설치에서 복원할 수 있다.
아카이브 파일을 백업 디렉터리에 두고 같은 복구 키로 기동하면 관리자 화면의 백업 목록에
`discovered`로 나타나며, 그대로 복원하면 설정과 기존 API 키가 함께 돌아온다.

아카이브는 스키마의 모든 테이블을 담되, 일부러 제외하는 것만 남긴다 — 진행 중인 색인
generation의 staging 테이블, FTS 그림자 테이블(원본에서 재생성된다), 세션·로그인 흐름·
복구 토큰(복원은 활성 세션을 폐기하는 것이 의도된 동작이다), 복원을 수행하는 설치 자신의
마이그레이션 이력과 백업 목록. 어느 쪽에도 속하지 않는 테이블이 생기면 시험이 실패한다.
이 목록은 SQLite↔PostgreSQL 데이터베이스 이관에도 같은 것이 쓰인다.
...
- 거절된 API 키는 `audit_logs`에 `apikey.auth` / `failure`로 남는다. 주소마다 첫 번째,
  열 번째, 백 번째, 천 번째 시도에만 한 줄을 쓰고 각 줄이 그 시점까지의 시도 수를
  담으므로, 한 번의 탐색도 보이고 대량 탐색도 감사 로그를 채우지 않는다. 기록되는 것은
  키의 prefix이며 나머지 값은 남기지 않는다. 할당량 초과는 거절된 자격증명이 아니므로
  여기에 남지 않는다 — 바쁜 에이전트가 침입자로 보이면 안 된다. 주소는 4,096개까지
  개별로 세고 그보다 넓게 퍼진 탐색은 하나의 항목으로 합쳐 세며, 그 경우 감사 줄에
  `countedUnder`가 함께 남는다. 세는 비용은 시도 수와 무관하게 일정하다.
...
- 등록된 경로에 저장소가 없으면(404) 색인 실패 메시지가 **세 가지 가능성을 모두** 밝힌다 —
  이름 변경·삭제·서비스 계정 권한 회수. Bitbucket과 GitLab 모두 볼 수 없는 저장소에
  403이 아니라 404를 돌려주므로, 소스가 보낸 "존재하지 않는다"는 문장만으로는 셋을
  구분할 수 없다. 저장소 항목은 **소스 ID로 식별**하므로, 현재 이름으로 다시 등록하면
  새 항목이 생기지 않고 기존 항목이 갱신된다. 메시지에 그 ID가 함께 나온다.
...
키 이하로 얕아지는 줄 전까지 본문을 줄마다 치환해 줄 수를 그대로 둔다. 리스트에서는
대시 앞 들여쓰기가 아니라 키가 시작하는 열을 블록 경계로 사용해 같은 항목의 형제 필드를 보존한다. 호스트는
