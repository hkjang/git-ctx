...
서명뿐 아니라 저장된 설정 암호화 키와 백업 아카이브 봉인의 뿌리이기도 하다.

SQLite DSN에 `_foreign_keys`와 `_busy_timeout`이 없으면 각각 `on`과 `5000`으로
채워서 연다. SQLite는 연결마다 외래 키를 기본으로 끄고, 그러면 스키마의 모든
`ON DELETE CASCADE`가 아무 일도 하지 않는다. 운영자가 명시한 값은 그대로 둔다. 다음과
...
`get-repository-map` 응답에는 해당 ref 가 **직접 선언한** 서드파티 의존성이 함께
나옵니다. 처음 여는 저장소에 코드를 쓸 때 어떤 라이브러리를 써야 하는지는 그
저장소가 이미 쓰는 것이 정하기 때문입니다. 전이 의존성과 락파일 해석 결과는
제외합니다 — 방향을 잡는 데는 소음입니다. 25개까지 보여 주고 그보다 많으면 남은
개수를 밝힙니다. 전체 목록은 `find-dependency-usage` 가 답합니다.

## 웹훅 수신 내역

`GET /api/v1/admin/webhook-events` 와 소스·색인 화면의 **웹훅 수신** 카드는 소스
서버가 실제로 보낸 이벤트를 보여 줍니다. 자동 색인이 되지 않을 때 원인은 보통 세
가지인데 — 훅 미설정, 이 플랫폼에 없는 저장소를 가리키는 훅, 색인 자체의 실패 —
앞의 둘은 화면에 아무 흔적이 없었습니다.

거부된 이벤트도 사유와 함께 기록합니다. 예를 들어 등록되지 않은 저장소의 훅은
`rejected` 로 남고, 보낸 쪽이 사용한 저장소 식별자가 함께 표시되어 어느 저장소를
등록해야 하는지 바로 알 수 있습니다. 서명 검증에 실패한 요청은 기록하지 않습니다.
인증되지 않은 요청은 임의의 외부 요청이므로 남기면 그 자체가 저장소 낭비입니다.

## 의존성 인벤토리

색인할 때 저장소의 의존성 매니페스트를 함께 읽어 `repository_packages` 에 적재합니다.
...
다른 버전)도 각각 기록합니다. 취약한 전이 사본이 최상위 선언과 별개로 존재할 수
있기 때문입니다.

- **내용 색인 정책과 무관하게** 매니페스트는 읽습니다. `.xml` 이나 `.json` 을 색인
  대상에서 제외한 저장소만 인벤토리에서 빠지면, "사용처 없음" 이 사실과 달라지기
  때문입니다. 한 ref 당 매니페스트 60개로 제한합니다.
- 매니페스트를 읽지 못하면 색인은 실패시키지 않고 작업 경고로 남깁니다. 조용한
  누락은 보안 공지 대응에서 잘못된 안심으로 이어집니다.
- 재색인 시 해당 ref 의 인벤토리를 **교체**합니다. 매니페스트에서 빠진 패키지는
  즉시 보고에서도 사라집니다.
- Maven 의 `${property}` 버전은 같은 파일의 properties 로 해석하고, 해석되지 않으면
  플레이스홀더 대신 "미지정" 으로 남깁니다. go.mod 의 `// indirect` 는 전이 의존성으로
  구분합니다.

응답에는 답변이 참조한 저장소의 **색인 최신성**도 포함됩니다. 한 달 전에 색인된
저장소는 그 사이 버전을 올렸을 수도, 반대로 영향 버전을 받았을 수도 있습니다.
"안전" 판정이 오래된 색인에 근거한 것이라면 그 사실이 진단에 표시됩니다.

`find-dependency-usage` 도구와 콘솔의 **Dependency Usage** 카드가 이 데이터를
버전별로 묶어 보여 줍니다. 보안 공지의 **수정 버전(`fixedIn`)** 을 함께 넘기면 각
버전 묶음을 **영향·안전·판정 불가** 로 분류합니다. 범위는 위로만 해석되므로 한 방향으로만
판정합니다. `>=2.17.1` 처럼 하한이 수정 버전 이상이면 안전이고, `^18.2.0` 처럼
하한이 수정 버전보다 낮으면 실제 해석 결과에 달렸으므로 **판정 불가**입니다(락파일이
있으면 그것으로 결정됩니다). `latest` 같은 부동 버전도 **안전으로 접지 않고
판정 불가로 남깁니다** — 잘못된 안심이 보안 공지 대응에서 가장 비싼 실수이기
때문입니다. 한 저장소가 영향 버전과 수정 버전을 함께 선언하면 영향으로 분류합니다. 보안 공지 대응("영향 버전을 쓰는 저장소")과 업그레이드
계획("몇 개 버전이 공존하는가")이 주 용도입니다. 아직 아무것도 색인되지 않은
상태와 정말 사용처가 없는 상태를 응답에서 구분합니다.

### 카탈로그 전체 인벤토리

`GET /api/v1/admin/dependency-inventory` 와 소스·색인 화면의 **의존성 인벤토리**
카드는 패키지 이름을 몰라도 조직이 무엇에 의존하는지 보여 줍니다. 정렬 기준은
**버전 분산** 입니다. 한 버전으로 널리 쓰는 라이브러리는 결정할 것이 없고,
저장소마다 버전이 갈린 라이브러리가 표준화 대상이기 때문입니다.

목록에는 **매니페스트가 색인된 저장소 비율**을 항상 함께 표시합니다. 일부만 색인된
상태의 짧은 목록을 "우리는 별로 안 쓴다" 로 읽으면 판단이 통째로 틀리기 때문입니다.
집계는 호출자의 ACL 범위 안에서만 이루어집니다.
