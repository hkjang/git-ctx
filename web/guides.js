...
(function (root, factory) {
  ...
  const guides = {
    ...
    "settings-workflow": {
      title: "설정 저장·이력·이관 가이드",
      audience: "platform-admin, 위임된 설정 관리자",
      summary:
        "설정 화면은 저장만 하는 곳이 아니라, 누가 무엇을 언제 바꿨고 되돌릴 수 있는지를 함께 관리하는 곳입니다. 동시 편집 충돌, 검증 실패, 비밀값 처리, 환경 간 이관을 여기서 처리합니다.",
      sections: [
        {
          title: "1. 저장이 막히는 세 가지 경우",
          table: {
            head: ["증상", "원인", "해결"],
            rows: [
              ["403 권한 없음", "해당 영역의 위임 역할이 없습니다.", "ACL 가이드의 플랫폼 역할 표를 확인하세요."],
              ["409 버전 충돌", "화면을 연 뒤 다른 관리자가 먼저 저장했습니다.", "안내에 따라 다시 불러온 뒤 변경 내용을 다시 입력하세요. 덮어쓰기를 막기 위한 정상 동작입니다."],
              ["400 검증 실패", "대상 서버에 연결하지 못했습니다.", "주소·인증서를 확인하고, 점검 중이라면 [검증 건너뛰고 저장]으로 진행하세요. 건너뛴 사실은 감사 로그에 남습니다."],
            ],
          },
        },
        {
          title: "2. 비밀값 다루기",
          body: [
            "저장된 비밀값은 항상 ******** 로 표시됩니다. 그대로 저장하면 기존 값이 유지되고, 새 값을 입력하면 교체됩니다.",
            "값을 없애려면 [저장된 값 지우기] 를 누르세요. 입력란을 비우는 것만으로는 '변경 없음'과 구분되지 않아 기존 값이 유지됩니다.",
            "저장된 값이 없는데 ******** 가 들어오면 그 항목은 저장하지 않고 '입력 필요' 로 알려 줍니다. 마스크가 그대로 비밀값이 되는 사고를 막기 위해서입니다.",
          ],
        },
        {
          title: "3. 이력과 되돌리기",
          steps: [
            "[변경 이력] 에서 버전 목록을 엽니다.",
            "[비교·되돌리기] 로 현재 값과의 차이를 먼저 확인합니다. 비밀값은 '변경됨'까지만 표시됩니다.",
            "되돌리면 과거 내용이 새 버전으로 기록됩니다. 이력은 지워지지 않으므로 되돌리기도 다시 되돌릴 수 있습니다.",
          ],
        },
        {
          title: "4. 환경 간 이관",
          body: [
            "[설정 내보내기·가져오기] 에서 전체 설정을 JSON 한 파일로 받습니다. 비밀값은 파일에 포함되지 않습니다.",
            "가져오기는 기본이 미리보기입니다. 영역별로 무엇이 바뀌는지 확인한 뒤 [미리본 내용 적용] 을 누르세요.",
            "가져오는 환경에 이미 저장된 비밀값은 유지되고, 없는 항목은 '입력 필요' 로 표시됩니다. 적용 후 해당 영역에서 실제 값을 입력하세요.",
          ],
        },
      ],
    },
    ...
    mcp: {
      ...
      sections: [
        ...
        },
        {
          ...
          body: [
            "개별 도구의 사용 여부, Timeout, 캐시, 응답 예산은 관리자 → MCP 도구 운영 화면에서 조정합니다.",
            "응답 예산은 한 번의 응답이 차지할 수 있는 최대 바이트입니다. 0 이면 기본값 24KB 를 쓰고, 초과하면 결과 경계에서 잘린 뒤 '얼마나 남았는지'와 '어떻게 좁힐지'를 함께 반환합니다. 클라이언트는 maxBytes 인자로 더 작게만 요청할 수 있습니다.",
        ...
        },
        {
          ...
          table: {
            head: ["지표", "판단 기준", "조치"],
            rows: [
              ["잘림 비율", "20% 이상이면 에이전트가 같은 질문을 반복합니다.", "응답 예산을 두 배로 올리거나 기본 limit 을 낮춥니다."],
              ["p95 지연", "Timeout 의 80% 를 넘으면 응답이 잘린 채 반환됩니다.", "Timeout 을 올리거나 원격 폴백 대상 저장소를 색인합니다."],
              ["빈 응답률", "40% 이상이면 검색이 아니라 색인·ACL 문제입니다.", "'답하지 못한 질문' 목록에서 대상 저장소를 찾아 색인 진단을 확인합니다."],
              ["검색 경로", "remote 비중이 높으면 색인이 따라가지 못한 것입니다.", "해당 저장소의 색인 주기를 줄이거나 재색인합니다."],
              ["캐시 적중", "0% 이고 평균 지연이 길면 캐시가 꺼져 있습니다.", "Cache 를 30 초부터 올려 봅니다."],
            ],
          },
        },
        {
          title: "4. 호출 감사와 X-ray",
          body: [
            "호출 감사 로그는 시각·사용자·API 키·세션·클라이언트·질의 요약·검색 경로·결과 수·응답 크기를 남깁니다. 질의는 비밀값 마스킹 후 300자로 잘라 저장합니다.",
            "보관 기간은 운영 설정의 MCP 호출 보관일을 따릅니다. CSV 로 내보내면 내보낸 사실 자체도 감사 로그에 남습니다.",
            "각 호출의 [X-ray] 버튼은 단계별 실행 기록을 펼칩니다. 단계마다 후보 수와 통과 수가 있어, 결과가 사라진 지점을 바로 짚을 수 있습니다. ACL 단계가 0이면 권한, 색인 단계가 0이면 미색인, source-query 가 후보는 있는데 통과가 0이면 그 저장소 안에 매칭이 없었다는 뜻입니다.",
            "같은 세션의 호출 순서도 함께 보여 줍니다. search-code 가 비어서 search-semantic 으로 넘어갔는지 같은 에이전트의 판단 흐름을 그대로 읽을 수 있습니다.",
          ],
        },
        {
          title: "5. 세션과 자가 점검",
          body: [
            "세션 표는 에이전트 대화 단위로 묶습니다. 호출 하나씩 보면 정상인데 대화 전체로는 답을 못 얻고 끝난 경우를 찾기 위해, 마지막 호출이 빈 응답이나 오류였던 대화를 위로 올립니다. 호출 흐름(search-code → find-file)이 그대로 보이므로 에이전트가 무엇을 시도했는지 알 수 있습니다.",
            "[자가 점검 실행] 은 저장된 설정을 검사하는 것이 아니라, 지금 로그인한 관리자의 ACL 주체로 코드 검색·파일명 검색·의미 검색을 실제로 실행합니다. 실패하면 단계별 후보→통과 수와 함께 다음에 할 일을 알려 줍니다.",
          ],
        },
      ],
    },
    ...
    search: {
      title: "검색 실행 모드·가중치 가이드",
      audience: "platform-admin, search-admin",
      summary:
        "임베딩을 사용할지, 장애 시 검색을 계속할지 결정하고 키워드·벡터 검색의 가중치와 결과 수를 조정합니다.",
      sections: [
        {
          title: "1. 검색 실행 모드",
          table: {
            head: ["모드", "동작", "권장 환경"],
            rows: [
              ["키워드 전용", "임베딩 생성·벡터 DB 조회 없이 로컬 키워드와 Bitbucket/GitLab 검색 API 사용", "모델이 없거나 임베딩을 정책상 사용하지 않는 환경"],
              ["하이브리드 + 자동 폴백", "정상일 때 벡터를 결합하고 모델·벡터 데이터 장애 시 키워드 검색으로 자동 전환", "일반 운영 환경의 권장값"],
              ["하이브리드 필수", "임베딩 모델 또는 데이터가 없으면 명확한 오류 반환", "의미 검색 품질을 반드시 보장해야 하는 통제 환경"],
            ],
          },
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              ["재순위화 후보 수", "30", "Reranker 사용 시 상위 몇 건을 재정렬할지 정합니다. 재순위는 query-docs 답변에만 적용됩니다."],
              ["최소 임베딩 커버리지", "80%", "접근 가능한 ref의 벡터 비율이 이 값보다 낮으면 부분 벡터 결과의 편향을 막기 위해 전체 질의를 키워드·원격 검색으로 전환합니다."],
              ["Circuit 연속 실패", "2회", "모델 장애가 이 횟수만큼 연속되면 Cooldown 동안 호출을 즉시 차단합니다."],
              ["Circuit 재시도 대기", "60초", "대기 후 한 요청만 모델을 시험하고 성공하면 자동 복구합니다."],
              ["질의 벡터 캐시", "120초", "동일 질의에서 원격 후보 결합과 최종 점수 계산이 같은 임베딩을 재사용합니다. 동시에 들어온 같은 질의도 한 번의 모델 호출로 병합됩니다."],
            ],
          },
        },
        {
          title: "3. 변경과 재색인",
          body: [
            "실행 모드를 저장하면 MCP 캐시 키가 즉시 바뀌므로 이전 모드의 응답이 재사용되지 않습니다.",
            "색인된 ref는 자동으로 재색인 큐에 등록됩니다. staging 세대가 완료될 때까지 기존 검색 데이터는 유지됩니다.",
            "Embedding provider·모델·revision이 바뀌면 구형 벡터는 즉시 검색 후보에서 격리되고, 호환되지 않는 ref가 자동 재색인됩니다. 완료 전에는 질의 전체가 키워드·소스 Query API로 폴백합니다.",
            "자동 폴백 모드에서는 임베딩 endpoint가 실패해도 문서·코드·심볼 색인은 완료되며 작업 화면에 경고가 남습니다.",
            "검색 설정과 벡터 DB 탭의 런타임 상태에는 요청 정책·실제 동작 모드·호환 커버리지·불일치 ref·Circuit·캐시 적중·동시 요청 병합이 함께 표시됩니다.",
          ],
        },
        {
          title: "4. 변경 후 확인",
    ...
    model: {
      ...
      audience: "platform-admin, search-admin",
      summary:
        "실제 의미 검색에는 사내 OpenAI 호환 임베딩 엔드포인트를 사용합니다. 모델 미설정 상태에서는 검색 정책에 따라 키워드 검색이 계속 동작합니다.",
      sections: [
        {
          title: "1. Embedding",
          steps: [
            "Provider 를 openai-compatible 로 바꾸고 API URL, API Key, 모델명을 입력합니다. local은 이전 설정 호환용이며 운영 의미 검색에는 사용되지 않습니다.",
            "[연결 테스트·검증] 은 실제 임베딩 1회를 호출해 차원과 응답을 확인합니다.",
            "모델을 바꾸면 저장된 revision과 새 설정을 비교해 기존 벡터를 즉시 격리하고 ref별 재색인을 자동 예약합니다. 상태 화면에서 호환 커버리지가 100%로 회복됐는지 확인하세요.",
          ],
        },
        {
          title: "2. Reranker",
      ...
      sections: [
        ...
        },
        {
          ...
          steps: [
            ...
            "메타 DB에 저장된 임베딩을 500개 단위로 벡터 DB에 적재합니다. 현재 모델 revision과 다른 벡터는 검색에서 제외되므로 먼저 자동 재색인 완료 여부를 확인하세요.",
            "pgvector ↔ Milvus 전환도 같은 방법입니다. 새 대상으로 설정을 바꾸고 재적재하면 됩니다.",
            "이후 색인 작업이 끝날 때마다 해당 ref의 벡터가 자동으로 갱신됩니다.",
          ],
          notice: "임베딩 모델을 바꾸면 구형 벡터는 자동 격리되고 ref별 재색인이 예약됩니다. 재색인 완료 후 벡터 재적재를 실행해 외부 벡터 DB도 새 revision으로 맞추세요.",
        },
      ],
      troubleshooting: [
        {
        ...
        },
        {
          ...
          cause: "설정 이후 색인된 ref만 자동 반영되거나, 현재 모델 revision과 다른 구형 벡터가 격리되어 있습니다.",
          fix: "불일치 ref의 자동 재색인 완료를 확인한 뒤 [벡터 재적재]를 실행하세요.",
        },
        {
    ...
    backup: {
      ...
      audience: "platform-admin",
      summary:
        "GIT_CTX_RECOVERY_KEY 에서 파생한 키로 암호화된 예약 백업을 만들고 보존 개수를 관리합니다. 같은 복구 키를 가진 새 설치에서 복원할 수 있으므로 복구 키는 백업 볼륨과 분리해 보관합니다. 복원하면 모든 세션이 폐기됩니다.",
      sections: [
        {
          title: "1. 설정",
          table: {
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              ["find-dependency-usage", "보안 공지·업그레이드로 라이브러리 사용처를 찾을 때", "저장소·버전 묶음과 수정 버전 기준 영향/안전/판정 불가 분류"],
    ...
    indexing: {
      ...
      sections: [
        {
          title: "0. 연동 상태(서킷 브레이커)",
          body: [
            "소스 서버 호출이 연속으로 실패하면 잠시 호출을 멈추고 색인된 결과만으로 답합니다. 죽은 서버를 저장소마다 다시 호출하느라 MCP 도구 타임아웃을 모두 소진하는 것을 막기 위해서입니다.",
            "이 상태에서도 검색은 실패하지 않습니다. 응답의 Notes 와 진단 메시지에 '연동이 일시 중단되었습니다'와 재시도 시각이 표시됩니다.",
            "인증 오류(401·403)는 기다려도 낫지 않으므로 즉시 중단합니다. 토큰을 고친 뒤 [지금 재시도] 를 누르면 바로 다시 호출합니다.",
          ],
          table: {
            head: ["상태", "의미", "조치"],
            rows: [
              ["closed", "정상 호출 중", "없음"],
              ["degraded", "실패가 누적되는 중", "소스 서버 상태와 네트워크를 확인하세요."],
              ["open", "호출 중단, 색인 결과로만 응답", "원인을 고친 뒤 [지금 재시도]"],
              ["half-open", "복구 확인용 호출 1건만 허용", "잠시 기다리면 자동으로 판정됩니다."],
            ],
          },
        },
        {
        ...
        },
        {
          ...
          table: {
            ...
            rows: [
              ...
              ["정체(stalled)", "10분 이상 running", "소스 서버·임베딩 응답 확인. 15분 리스가 지나면 자동으로 다시 큐에 들어갑니다"],
            ],
          },
        },
        {
          ...
          body: [
            "작업 시작 시 임베딩 엔드포인트에 1건 probe를 보냅니다. URL·모델명·API Key가 틀리면 파일을 내려받기 전에 즉시 실패하고 원인이 작업 오류란에 남습니다.",
      ...
      troubleshooting: [
        ...
        {
          symptom: "저장소가 계속 '미색인' 상태",
          cause: "작업이 대기 중이거나, 이전 실행이 중단된 채 running으로 남아 있거나, 실패했습니다.",
          fix: "소스·색인 화면의 [색인 진단] 표에서 상태와 원인·조치를 확인하세요. 정체 작업은 15분 리스 후 자동 재큐되고, 실패 작업은 [재시도]로 다시 실행합니다.",
        },
        {
    ...
    database: {
      ...
      sections: [
        ...
        },
        {
          ...
          notice:
            "설정 암호화 키와 API-key pepper 는 데이터베이스에 저장되며 GIT_CTX_RECOVERY_KEY 에서 파생한 키로 감싸입니다. 따라서 Bootstrap DSN 문자열은 자유롭게 바꿀 수 있고, 지켜야 하는 것은 복구 키입니다. v0.66.0 이전에 만들어진 설치는 한 번만 원래 DSN 으로 기동하면 키가 기록됩니다.",
        },
      ],
    },
  };
