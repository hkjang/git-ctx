...
var registry = []tool{
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"query"}, "properties": map[string]any{
			...
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 50, "description": "Maximum results to return (1-50)"}}},
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"pattern"}, "properties": map[string]any{
			...
			"repository": map[string]string{"type": "string", "description": "Optional repository slug or library ID"},
			"ref":        map[string]string{"type": "string", "description": "Optional branch or tag"},
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 200, "description": "Maximum results to return (1-200)"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"target"}, "properties": map[string]any{
			...
			"sourceType": map[string]any{"type": "string", "enum": []string{"bitbucket", "gitlab", "confluence", "jira"}, "description": "Optional source filter"},
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 300, "description": "Maximum results to return (1-300)"}}},
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{
			...
			"libraryId":  map[string]string{"type": "string", "description": "Optional library scope"},
			"repository": map[string]string{"type": "string", "description": "Optional repository slug or library ID"},
			"state":      map[string]any{"type": "string", "enum": []string{"all", "open", "merged", "closed"}, "description": "Request state filter; defaults to all"},
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 100, "description": "Maximum results to return (1-100)"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"libraryId", "symbol"}, "properties": map[string]any{
			...
			"ref":       map[string]string{"type": "string", "description": "Optional branch or tag; the repository default branch is used when omitted"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"libraryId", "symbol"}, "properties": map[string]any{
			"libraryId": map[string]string{"type": "string", "description": "Context7-compatible library ID of the repository the symbol lives in"},
			"symbol":    map[string]string{"type": "string", "description": "Symbol name or qualified name whose callers and callees are traced"},
			"ref":       map[string]string{"type": "string", "description": "Optional branch or tag; the repository default branch is used when omitted"},
			"limit":     map[string]any{"type": "integer", "minimum": 1, "maximum": 200, "description": "Maximum results to return (1-200)"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"libraryId", "baseRef", "headRef"}, "properties": map[string]any{
			"libraryId": map[string]string{"type": "string", "description": "Context7-compatible library ID of the repository being compared"},
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"pack", "query"}, "properties": map[string]any{
			...
			"query": map[string]string{"type": "string", "description": "Task context used to rank the entries in the pack"}}},
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"query"}, "properties": map[string]any{
			"query":     map[string]string{"type": "string", "description": "Incident, procedure or system the runbook would be about"},
			"libraryId": map[string]string{"type": "string", "description": "Optional library scope"},
			"limit":     map[string]any{"type": "integer", "minimum": 1, "maximum": 50, "description": "Maximum results to return (1-50)"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"libraryIds", "query"}, "properties": map[string]any{
			"libraryIds": map[string]any{"type": "array", "minItems": 1, "maxItems": 20, "items": map[string]string{"type": "string"},
				"description": "One to twenty Context7-compatible library IDs to export"},
			"query": map[string]string{"type": "string", "description": "Question the exported context has to answer"}}},
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"libraryId", "query"}, "properties": map[string]any{
			"libraryId": map[string]string{"type": "string", "description": "Context7-compatible library ID of the repository whose result is explained"},
			"query":     map[string]string{"type": "string", "description": "The search text whose ranking is being explained"},
			"ref":       map[string]string{"type": "string", "description": "Optional branch or tag; the repository default branch is used when omitted"},
			"limit":     map[string]any{"type": "integer", "minimum": 1, "maximum": 50, "description": "Maximum results to return (1-50)"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"path"}, "properties": map[string]any{
			...
			"repository": map[string]string{"type": "string", "description": "Optional repository slug or library ID"},
			"ref":        map[string]string{"type": "string", "description": "Optional branch or tag"},
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 20, "description": "Maximum results to return (1-20)"}}},
		usesLibraryID: true,
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "required": []string{"name"}, "properties": map[string]any{
			...
			"limit":      map[string]any{"type": "integer", "minimum": 1, "maximum": 500, "description": "Maximum results to return (1-500)"}}},
	...
	},
	{
		...
		schema: map[string]any{"type": "object", "additionalProperties": false, "properties": map[string]any{
			"status": map[string]any{"type": "string", "enum": []string{"pending", "running", "completed", "failed"}, "description": "Job state filter; all states are returned when omitted"},
			"limit":  map[string]any{"type": "integer", "minimum": 1, "maximum": 100, "description": "Maximum results to return (1-100)"}}},
